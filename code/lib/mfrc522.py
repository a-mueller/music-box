...
class MFRC522:
	...
	def __init__(self, spi: SPI, rst: Pin, cs: Pin):
	...
	def power_off(self):
		self._wreg(0x02, 0x10)
		self._wreg(0x02, 0x00)
		self.rst.value = False

	def _ticks_diff(self, ticks1, ticks2):
	...
	def _tocard(self, cmd: int, send):
		...
		tick_start = supervisor.ticks_ms()
		completed = False
		while (self._ticks_diff(supervisor.ticks_ms(), tick_start) < 52 and not completed):
			n = self._rreg(0x04)			# ComIrqReg[7..0] bits are: Set1 TxIRq RxIRq IdleIRq HiAlertIRq LoAlertIRq ErrIRq TimerIRq
			if (n & wait_irq):
				completed = True
